...
import Noise_test as wn
# Ouvrir
#img = Image.open("01_test.tiff")
def main_segmentation(image_name="39_training.tif",wns=0,filtre="f",change_contraste=0.9,chosen_tau=0.5,alpha=0.5,beta=3, input_as_array=False ,pixels_array=None):
    if input_as_array == False :
        ...
        def frangi(l1, l2):
            if (l2 < 0) : 
                ...
                couleur = int(np.nan_to_num(255*np.exp(-r*r/(2*alpha*alpha))*(1-np.exp(-s*s/(2*beta*beta))), nan=0.0))                
                return couleur
       ...
       # calcul_gris()
        ...
        v2=np.abs(calcul_vecteur_empirique_d_analyse_en_composante_principale(result,pixels,seuil=0.2))
        ...
        calcul_gris()
        if wns>0:
            gris = wn.add_white_noise(gris,sc=wns)
        gris=change_contraste*gris
        def convolution2(s):
            h11 = ndimage.gaussian_filter(gris, sigma=(s,0.7*s), order=(2, 0))
            h22 = ndimage.gaussian_filter(gris, sigma=(s,0.7*s), order=(0, 2))
            h12 = ndimage.gaussian_filter(gris, sigma=(s,0.7*s), order=(1, 1))
        ...
        img.save(f"filtre_frangi_nouvelle_gaussiène alpha= {alpha} , beta ={beta}.png")
